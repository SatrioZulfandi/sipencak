        ...
        <div class="table-responsive">
            <table class="table-custom">
                ...
                <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-sm btn-outline-primary rounded-pill px-3 py-1 fw-bold btn-riwayat" 
                                            data-id="<?= $mhs['id'] ?>" data-nim="<?= esc($mhs['nim']) ?>" data-nama="<?= esc($mhs['nama']) ?>"
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="9" class="text-center py-5 text-muted">Data mahasiswa tidak ditemukan.</td>
                        </tr>
                    <?php endif ?>
                </tbody>
            </table>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.btn-riwayat').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // Fetch data
            fetch(`<?= base_url('operator/mahasiswa/riwayat') ?>/${idMahasiswa}`)
                .then(res => res.json())
                .then(data => {
